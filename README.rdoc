...
== Required Gems
 ...
 aegis - manage permissions for user accounts in a central place. 
 active_shipping - package tracking
 chronic (pending) - natural language Date/Time parser. (also required for BackgrounDRb)
 packet (pending) - requirement for BackgrounDRb


== Required and included plugins

 jrails (pending) - jquery helper methods 
	(trying to not have any inline javascript so may never really use jrails' methods.)
 html_test - html testing and validation
 aegis_extension - modifications, updates and patches for aegis gem.
 backgroundrb (not yet / pending) - background job server and scheduler
